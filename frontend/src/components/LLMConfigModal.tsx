...
const MODEL_CAPABILITY_KEYS: CapabilityKey[] = ['tts', 'embedding', 'image', 'video'];
...
const LLMConfigModal: React.FC<LLMConfigModalProps> = ({ visible, onClose }) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const request: LLMConfigRequest = {
        id: editingConfig?.id,
        provider,
        configName: editingConfig?.configName || buildInternalConfigName(provider, values),
  ...
  const buildInternalConfigName = (provider: string, values: Record<string, string | undefined>) => {
    const configuredModel = values.model
      || values.ttsModel
      || values.embeddingModel
      || values.imageModel
      || values.videoModel
      || 'model';
    const modelPart = configuredModel.trim().replace(/\s+/g, '-');
  ...
  const hasSpecializedModelCapability = activeCapabilities.some((key) =>
    MODEL_CAPABILITY_KEYS.includes(key)
  );
  ...
  const addCapability = (key?: CapabilityKey) => {
    ...
    if (MODEL_CAPABILITY_KEYS.includes(key)) {
      form.setFields([{ name: 'model', errors: [] }]);
    }
      ...
      >
        ...
        >
          ...
          >
            ...
            <Form.Item
              name="model"
              label={`默认 LLM 模型${hasSpecializedModelCapability ? '（可选）' : ''}`}
              rules={[
                {
                  validator: (_, value) =>
                    value?.trim() || hasSpecializedModelCapability
                      ? Promise.resolve()
                      : Promise.reject(new Error('请输入默认 LLM 模型，或添加 TTS、向量、图片或视频模型'))
                }
              ]}
              extra={hasSpecializedModelCapability
                ? '仅使用下方专项能力时可留空。'
                : '未添加专项模型能力时必填。'}
            >
